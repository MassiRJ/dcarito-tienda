...
// --- ⚙️ CONFIGURACIÓN DE TU NEGOCIO (NUEVO) ---
const PRECIO_DELIVERY_BASE = 10; // Costo del delivery
const META_PARA_GRATIS = 3;      // Cantidad para que sea gratis
...
export default function Tienda() {
  ...
  const [tipoEntrega, setTipoEntrega] = useState<"tienda" | "delivery">("tienda"); // 👈 NUEVO: Estado de entrega
  ...
  const calcularCostoEnvio = () => {
    ...
    if (carrito.length >= META_PARA_GRATIS) return 0; // Gratis si lleva 3 o más
  ...
  // --- ✅ WHATSAPP MEJORADO (CON ENTREGA Y FOTOS) ---
  const checkoutWhatsApp = () => {
    ...
    carrito.forEach((item) => {
      const linkFoto = item.imagen ? urlFor(item.imagen).width(300).url() : "Sin foto";
      mensaje += `🛍️ *${item.nombre}* (S/ ${item.precio})\n`;
      mensaje += `   Ver Foto: ${linkFoto}\n\n`;
    });

    mensaje += `--------------------------\n`;
    mensaje += `📦 *MÉTODO DE ENTREGA:* ${tipoEntrega === "tienda" ? "Recojo en Tienda" : "Delivery"}\n`;
    
    if (tipoEntrega === "delivery") {
        if (esGratis) mensaje += `🚚 *Costo Envío:* ¡GRATIS! (Promo)\n`;
        else mensaje += `🚚 *Costo Envío:* S/ ${envio}\n`;
    }
    
    mensaje += `💰 *TOTAL FINAL: S/ ${total}*\n`;
    mensaje += `--------------------------\n\n`;
    mensaje += `¿Me envían el QR de Yape/Plin para pagar?`;

    const url = `https://wa.me/${NUMERO_WHATSAPP}?text=${encodeURIComponent(mensaje)}`;
    window.open(url, '_blank');
  };

  const MARQUEE_TEXT = "🚚 ENVÍOS GRATIS LLEVANDO 3 PRENDAS • 💳 ACEPTAMOS YAPE Y PLIN • 🎁 REGALO POR COMPRAS > S/199 • ⚡ ENVÍO EXPRESS EN LIMA • ";
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 selection:bg-rose-100 selection:text-rose-900">
      ...
      {/* --- PANEL LATERAL DEL CARRITO (DRAWER) MODIFICADO --- */}
      {carritoAbierto && (
        <div className="fixed inset-0 z-[60] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            {/* --- SECCIÓN DE PAGOS Y ENVÍO (NUEVO) --- */}
            {carrito.length > 0 && (
              <div className="p-6 border-t border-gray-100 bg-gray-50">
                
                {/* Selector de Entrega */}
                <div className="mb-6">
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-3">Método de Entrega</p>
                  <div className="grid grid-cols-2 gap-2">
                    <button 
                      onClick={() => setTipoEntrega("tienda")}
                      className={`py-2 px-3 rounded-lg text-sm font-bold border transition-all ${tipoEntrega === "tienda" ? "bg-black text-white border-black" : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"}`}
                    >
                      🏪 Recojo (S/0)
                    </button>
                    <button 
                      onClick={() => setTipoEntrega("delivery")}
                      className={`py-2 px-3 rounded-lg text-sm font-bold border transition-all ${tipoEntrega === "delivery" ? "bg-black text-white border-black" : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"}`}
                    >
                      🛵 Delivery
                    </button>
                  </div>
                  ...
                  {tipoEntrega === "delivery" && (
                    <div className={`mt-3 p-3 rounded-lg text-xs font-bold text-center border ${carrito.length >= META_PARA_GRATIS ? "bg-green-100 text-green-700 border-green-200" : "bg-orange-50 text-orange-600 border-orange-100"}`}>
