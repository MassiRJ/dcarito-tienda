...
// Importamos la fuente Serif elegante
...
// ⚠️ NÚMERO DE LA DUEÑA ACTUALIZADO
const NUMERO_WHATSAPP = "51916932897"; 

// --- 🔥 BANNERS DE RESPALDO (Por si Sanity está vacío) ---
...
export default function Tienda() {
  ...
  const checkoutWhatsApp = () => {
    ...
    carrito.forEach((item) => {
      // Generamos el link de la foto
      const linkFoto = item.imagen ? urlFor(item.imagen).width(400).url() : "Sin foto disponible";
      
      mensaje += `🛍️ *${item.nombre}*\n`;
      mensaje += `   Precio: S/ ${item.precio}\n`;
      mensaje += `   Foto: ${linkFoto}\n\n`; // Link azul para que la dueña vea
    });

    mensaje += `--------------------------\n`;
    mensaje += `💰 *TOTAL A PAGAR: S/ ${calcularTotal()}*\n`;
    mensaje += `--------------------------\n\n`;
  ...
  const MARQUEE_TEXT = "🚚 ENVÍOS GRATIS A TODO EL PERÚ • 💳 ACEPTAMOS YAPE Y PLIN • 🎁 REGALO POR COMPRAS > S/199 • ";
  ...
  return (
    <div className={`min-h-screen bg-[#f8f5f0] font-sans text-gray-900 ${playfair.variable}`}>
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="p-6 border-t border-[#e8e0d5] bg-[#f8f5f0]">
              ...
              <button 
                ...
                className="w-full bg-green-600 text-white py-4 rounded-md font-bold uppercase tracking-widest hover:bg-green-700 transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2 shadow-lg"
              >
      ...
      {/* 3. HERO SLIDER (Híbrido y Optimizado Móvil) */}
